...
func TestScalarSel(t *testing.T) {
	json := FmtFile("../data/csv/Questions.json", 2)
...
func BenchmarkScalarSelX(b *testing.B) {
	json := FmtFile("../data/csv/data1.json", 2)
	for n := 0; n < b.N; n++ {
...
func TestJoin(t *testing.T) {
	jastrL := FmtFile("../data/csv/Modules.json", 2)
	jastrR := FmtFile("../data/csv/Substrands.json", 2)
	result, pairs := ArrJoin(jastrL, "substrand_id", jastrR, "substrand_id", "")
	for _, pair := range pairs {
		fPln(pair, "joined")
	}
	mustWriteFile("../data/csv/Modules-Substrands.json", []byte(result))
}
